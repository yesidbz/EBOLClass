...
export default class Docentes1 extends Component {
 constructor(props) {
 super(props);
 this.state = {
 docentes: [],
 ...
 let docentes = [];
 r.data.forEach(d => {
 const {iddocente,documento_docente,name,segundo_nombre,primer_apellido,segundo_apellido,celular,telefono,estado } = d;
 let obj = {
 iddocente,
 documento_docente,
name,
...
editar(iddocente){
  this.props.history.push(`/administrador/modificar/${iddocente}`);
}

detalle(iddocente){
  this.props.history.push(`/administrador/detalle/${iddocente}`);
}
