...
export default class area extends Component {
 ...
 let area = [];
 r.data.forEach(d => {
 const {id_area,nombre,estado } = d;
 let obj = {
 id_area,
 nombre,
 estado: estado === 1 ? "Activo" : "Inactivo",id_area,
 botones: [
 this.boton_estado(
 `btn btn-${estado === 1 ? "danger" : "success"}`,
 estado === 1 ? "Inhabilitar" : "Activar",id_area,
 ),
 <button onClick={()=>this.editar(id_area)} className="btn btn-primary">Editar</button>,
 <button onClick={()=>this.detalle(id_area)}  className="btn btn-warning" >Detalle</button>
...
detalle(id_area){
  this.props.history.push(`/Area/detalle/${id_area}`);
}
